...
class TTConfig:
    ...
    def __init__(self, path: str = "./", filename: str = "tt.ini") -> None:
        config_file = f"{path}/{filename}"
        self.config.read(config_file)
        #print(f"Config file !!: {config_file}")  # Debug print statement

        self.use_prod = self.config.get("Config", "use_prod") in (
            "True", "true", "yes", "t", "1", "y", "on",
        )
